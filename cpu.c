...
void start(Machine* machine, Instruction* instructions, int* memoriesSize) {
    startRAM(&machine->ram, memoriesSize[0]); // memoria principal (RAM)
    startCache(&machine->l1, memoriesSize[1]); // cache 1
    startCache(&machine->l2, memoriesSize[2]); // cache 2
    startCache(&machine->l3, memoriesSize[3]); // cache 3

    machine->instructions = instructions; 
    machine->hitL1 = 0; // inicializando tudo com 0
    machine->hitL2 = 0;
    machine->hitL3 = 0;
    machine->hitRAM = 0;
    machine->missL1 = 0;
...
void stop(Machine* machine) { // liberar as memorias alocadas
...
void executeInstruction(Machine* machine, int PC) {
    ...
    // Encontrar a linha na memória
    Line* line;
    WhereWasHit hit; // mudar isso aq
    metodoMapeamento metodo;
    
    switch (instruction.opcode) {
        case -1:
            printf("  > Ending execution.\n");
            break;
        case 0: // Pegando informação da RAM
            #ifdef PRINT_LOG
                printf("  > ATR (%4d) > ", add1.block);
            #endif
            
            line = MMUSearchOnMemorys(add2, machine, line, &hit, metodo); /* Searching block on memories */
            #ifdef PRINT_LOG
            ...
            break;
        case 1: // Soma
            line = MMUSearchOnMemorys(add1, machine, line, &hit, metodo); /* Searching block on memories */
            ...
            line = MMUSearchOnMemorys(add3, machine, line, &hit, metodo); /* Searching block on memories */
            #ifdef PRINT_LOG
                printf(" > BLOCK[%s.%d.%d](%4d|", convertToString(hit), add3.block, add3.word, line->block.words[add3.word]);
            #endif

            line->updated = true;
            line->block.words[add3.word] = word2 + word1; // o valor da posição 
            #ifdef PRINT_LOG
        ...
        case 2: // Subtração
            ...
            break;
        case 3: // Movendo informação na RAM
            line = MMUSearchOnMemorys(add1, machine, line, &hit, metodo); /* Searching block on memories */
            word1 = line->block.words[add1.word];
            #ifdef PRINT_LOG
...
void run(Machine* machine) {    
    int PC = 0; // Contador para cada instrução que foi executada/finalizada
